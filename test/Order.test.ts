...
test("Deve criar um pedido com 3 itens com um cupom de desconto expirado", function(){
    ...
    const order = new Order(cpf, new Date("2023-12-05"));
    order.addItem(new Item(1, "Musica","CD",30), 3);
    order.addItem(new Item(2, "Video","DVD",50), 1);
    order.addItem(new Item(3, "Video","VHS",10), 2);

    order.addCoupon(new Coupon("VALE20",20, new Date("2023-12-10")));
...
test("Deve criar um pedido com 3 itens com o cálculo de frete com a estratégia default", function(){
    ...
    const order = new Order(cpf, new Date("2023-12-05"), new DefaultFreightCalculator());
...
test("Deve criar um pedido com 3 itens com o cálculo de frete com a estratégia fixed", function(){
    ...
    const order = new Order(cpf, new Date("2023-12-05"), new FixedFreightCalculator());
    order.addItem(new Item(4, "Instrumentos Musicais","Guitarra",1000, 100, 30,10,3), 1) // 30 de frete;
    order.addItem(new Item(5, "Instrumentos Musicais","Amplificador",5000, 100, 50,50,20), 1) // ;
    order.addItem(new Item(6, "Acessórios","Cabo",3,10,10,10,0.9), 3);
